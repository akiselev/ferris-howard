...
| `use lean::Fp;` brings a Mathlib object alias into scope (`Fp<P>` is `ZMod P`); `use lean::Prime;` makes `where P: Prime` the binder `[Fact (Nat.Prime P)]` | confined — same import rule as F16, applied to objects and bounds instead of method spellings. Outside the import each name is an ordinary identifier. Landed M2 (A2.5) | design §6 |
| `use lean::C;` brings a class's F16 method spellings into scope (`p.dvd(a)` is `p ∣ a` only after `use lean::Dvd;`). Bridged as of M2: `Dvd`, `Abs`, `Pow`, `Function` | confined — Rust has trait imports and this is the same rule applied to notation spellings; outside the import `.` is plain dot notation, unchanged | F16, design §6 |
