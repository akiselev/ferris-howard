...
* **Rust digests it** — SHA-256 over the encoding's UTF-8 bytes, in `fh-atlas` / `fh check`, where a
...
| consumer | contract |
|---|---|
| C5 freeze/verify | re-elaborate from source in a fresh environment, encode, digest, compare against the frozen `fh-stmt-v1:sha256:…`. The rehearsal fixture (a weakened statement must fail) lands before first real use. |
