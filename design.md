...
Related work (recorded 2026-08-01): the nearest projects all run the *opposite* direction — hax and Aeneas verify real Rust programs by translating them into provers (Rust programs → Lean), whereas FH borrows Rust's syntax to write Lean mathematics; no Rust-syntax frontend for Lean exists. Alloy (embedded C in Lean) shows a full foreign grammar as Lean syntax categories works — and that LSP support for the embedded language is the perennially unfinished part, which is exactly why FH lives inside `.lean` files through M2 and defers standalone `.fh` to M3. Verso (Lean's documentation language) demonstrates the alternative-concrete-syntax genre with mechanically checked examples — the same corpus-as-spec discipline our golden tier enforces. Divergences from Rust are tracked in `differences.md` (the Ruling D page, started at M0).
...
**Amended 2026-08-01 (ADR-006, `research/codegen.md` §2): this preference is now load-bearing.** FH constructs are implemented as syntax→syntax macros expanding to Lean *surface* syntax wherever possible, because that makes `emit-lean` — publication-grade Lean with zero FH dependency — one macro-expansion step plus a formatter, with faithfulness by construction rather than by `Expr`-level delaboration, whose round-trips are lossy. The printable middle stage *is* the publication artifact. Two consequences are binding: an **emittable lint** (no FH node kind may survive expansion) fails at authoring time rather than at publication time, and a construct that genuinely needs `elab_rules` is either given a custom expander or kept out of the publishable subset deliberately. Post-elaboration *diagnostics* (linters, the F9 coercion audit) are not part of the translation and do not count as stage two.
...
Angle-bracket generics `<T, n: Nat>` become **implicit** binders — this matches Rust intuition perfectly, since Rust generics are inferred at call sites exactly as Lean implicits are. Ordinary parentheses parameters are **explicit** binders. `where` clauses become **instance** binders, also matching intuition (you never pass a trait impl explicitly in Rust either). Strict-implicits are rare enough for an attribute: `#[strict] <x: T>`. Turbofish `f::<Nat>(x)` maps to explicit instantiation `f (α := Nat) x` or `@f Nat x` — keep turbofish, it's beloved. **Amended 2026-08-01:** neither lowering is available to a stage-one macro. `@f Nat x` forces *instance* arguments explicit too and so fails on corpus F1's own `Grp::e::<G>()`; `f (α := Nat) x` needs the callee's binder name, which only elaboration knows. Turbofish is therefore deferred to a deliberate stage-two decision, and F1's escape is F10 ascription — `(Grp::e(): G)` — which is stage one and works today.
...
Four test tiers (amended 2026-08-01), all CI-enforced via `lake`: **golden expansion tests** (Rust-syntax input → expected Lean surface syntax, comparing hygiene-sanitized pretty-printed expansion via `#guard_msgs` with lax whitespace — cheap, catches macro regressions, and reviewable without running Lean mentally); **elaboration tests** (files that must elaborate with zero errors and a specified `sorry` count); **negative tests** (files that must *fail*, pinning the **exact** error message — exact-match is what the mechanism provides, re-baselining is a reviewed milestone-boundary event; a DSL's error behavior is API); and **span tests** (each feature asserts at least one diagnostic's FH-source position — span preservation is a hard requirement, so it is mechanically checked from M0, never audited after the fact). For the agent workstream this structure is the contract: every feature lands with all four tiers, and the golden tier doubles as the living syntax specification, which matters because "we come up with our syntax extensions as we go" needs a ratchet against accidental grammar drift. Suggest agents work from this doc section-by-section with §4 subsections as issue granularity, and that ambiguities discovered mid-implementation come back as PRs against *this document* first — the doc stays the source of truth, the grammar follows it. Authority order, made explicit after the 2026-08-01 reconciliation: this document and `corpus-review.md` are now consistent; if they ever disagree again, the corpus review's recorded rulings win until this document is amended, and the amendment lands before the code that depends on it. Corpus fixtures live at `Tests/corpus/gNN_*.lean` (FH-in-`.lean` per the §2 embedding decision; a `.fh` fixture form arrives with M3).
