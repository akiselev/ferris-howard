...
uv run scripts/atlas-mathlib-experiment.py    # any script; `import fh_atlas` just works
```
...
`import fh_atlas` with no `sys.path` surgery and no `PYTHONPATH`; `uv run` selects that
...
by looking: the artifact lands in `target/release/libfh_atlas.so`. That is worth checking
...
crates/fh-atlas-py/Cargo.toml` is the faster loop when iterating hard, but note that the
...
uv run crates/fh-atlas-py/tests/smoke.py        # --slice defaults to /tmp/mathlib-algebra.jsonl
```
...
* **`corpus.home`, `corpus.resolve`** — the residual/home-theory operations of §2. The
  `#fh_home` side of this is Lean, not Rust: there is nothing in `crates/fh-atlas` to bind.
...
**Bound here, not in `fh mcp`.** CLAUDE.md §6 wants a new query in five places, and the
fifth is the MCP tool list in `crates/fh-atlas/src/bin/fh-mcp.rs`. B4, B5 and B6 are absent
...
sweep in `crates/fh-atlas/examples/dictcheck.rs`, which asserts the transport lands on a
