...
fn load(path: &str) -> Vec<Row> {
    ...
    for line in text.lines() {
        ...
        let Ok(v) = fh_atlas::json::parse(line) else {
            continue;
        };
...
fn main() {
    ...
    let gt_path = std::env::args()
        .nth(2)
        .unwrap_or_else(|| "crates/fh-atlas/tests/gt-c-analogies.txt".into());
    ...
    let pool_shape: Vec<Option<String>> = pool
        .iter()
        .map(|&i| idx.skeleton_of(&rows[i].name, fh_atlas::skel::erase::Level::Shape))
        .collect();
    ...
    let shape_same: Vec<bool> = gt
        .iter()
        .map(|(a, b, _)| {
            let sa = idx.skeleton_of(&rows[*a].name, fh_atlas::skel::erase::Level::Shape);
            let sb = idx.skeleton_of(&rows[*b].name, fh_atlas::skel::erase::Level::Shape);
            sa.is_some() && sa == sb
        })
        .collect();
    ...
    for (pi, (a, b, _fam)) in gt.iter().enumerate() {
        ...
        // computed.
        let qshape = idx.skeleton_of(&rows[*a].name, fh_atlas::skel::erase::Level::Shape);
