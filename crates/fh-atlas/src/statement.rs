...
//!
//! The Lean side (`FerrisHoward/Atlas/Statement.lean`) produces the canonical *encoding*
//! of a statement; this module turns it into the frozen identity
//! `fh-stmt-v1:sha256:<hex>`. The split is deliberate — see `statement-hash.md`: the
...
/// `FerrisHoward/Atlas/Statement.lean`.
pub const ENCODING_VERSION: &str = "fh-stmt-v1";
...
/// The frozen identity of a canonical encoding: `fh-stmt-v1:sha256:<hex>`.
///
...
mod tests {
    ...
    /// `#fh_statement` on the pinned toolchain and pinned identically in
    /// `lean/Tests/Atlas/Statement.lean`. If one side changes, the other fails — that is
    /// the whole point of pinning it in both.
    const T1: &str = "fh-stmt-v1;pd(c(3:Nat,0),a(a(a(c(2:Eq,1,+(0)),c(3:Nat,0)),b0),b0))";

    /// `theorem w2 : ∀ n : Nat, n = 0 → n = n` — the weakened statement from the C5
    /// rehearsal. It must not collide with anything.
    const WEAKENED: &str = "fh-stmt-v1;pd(c(3:Nat,0),pd(a(a(a(c(2:Eq,1,+(0)),c(3:Nat,0)),b0),a(a(a(c(11:OfNat.ofNat,1,0),c(3:Nat,0)),n0),a(c(12:instOfNatNat,0),n0))),a(a(a(c(2:Eq,1,+(0)),c(3:Nat,0)),b1),b1)))";

    #[test]
    fn sha256_matches_published_vectors() {
        // Guards the digest itself, independently of anything FH-shaped.
        let mut hasher = Sha256::new();
        hasher.update(b"abc");
        assert_eq!(
    ...
    fn digest_is_prefixed_and_stable() {
        ...
        assert!(d.starts_with("fh-stmt-v1:sha256:"));
        assert_eq!(d, digest(T1).unwrap());
        assert_eq!(d.len(), "fh-stmt-v1:sha256:".len() + 64);
    }
    ...
    fn a_freeze_from_another_version_is_stale_not_different() {
        let frozen = "fh-stmt-v0:sha256:00";
        assert_eq!(
            verify(T1, frozen).unwrap(),
            Verdict::StaleFreeze {
                frozen_version: "fh-stmt-v0".to_string()
            }
        );
    }
    ...
    fn malformed_inputs_are_refused() {
        ...
        assert_eq!(digest("fh-stmt-v1;"), Err(StatementError::Malformed));
        assert_eq!(
            digest("fh-stmt-v99;b0"),
            Err(StatementError::UnknownVersion("fh-stmt-v99".to_string()))
        );
        assert_eq!(
            verify(T1, "fh-stmt-v1:md5:abcd"),
            Err(StatementError::MalformedFreeze)
        );
        assert_eq!(
            verify(T1, "fh-stmt-v1:sha256:nothex"),
            Err(StatementError::MalformedFreeze)
        );
    }
}
