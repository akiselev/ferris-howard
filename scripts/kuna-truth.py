...
2. **`to_class` must be a strict ancestor of `from_class` in the lattice the corpus
   records.** The lattice is read off parent projections exactly as `scripts/fh_home.py`
   ...
   reconstruction is *identical* to `fh_home.HomeIndex`'s on the algebra slice. Ancestry is
...
# ---------------------------------------------------------------------------
# The lattice — same rule as `fh_home.HomeIndex`, streamed so it fits the 4.8 GB closure.
#
# `fh_home` holds every row in memory to find the instance-binder heads; on the whole
...
# `fh_home.HomeIndex`'s class set and parent edges exactly, and it does (644 / 628).
# ---------------------------------------------------------------------------
...
def history(repo: str) -> list[dict]:
    ...
    """
    out = git(repo, "log", "--no-merges", "--format=@@FH@@%H%x01%ct%x01%s",
              "--numstat", "--", "*.lean")
    rows: list[dict] = []
    cur: dict | None = None
    for line in out.splitlines():
        if line.startswith("@@FH@@"):
            if cur is not None:
                rows.append(cur)
            sha, ts, subject = line[len("@@FH@@"):].split("\x01", 2)
