#!/usr/bin/env python3
"""Score `#fh_home_attempt` shard logs — the refuter lane's scorer.
...
controls (`attempt-plan.py`): `fh_plant_easy` must be PROVED, `fh_plant_hard` must be not
proved, `fh_plant_no_statement` must be NO STATEMENT. A shard whose plants are wrong or
...
LINE = re.compile(r"FH attempt `([^`]+)`: (\S+) -> (\S+): (.*)")
PLANTS = {
    "fh_plant_easy": "proved",
    "fh_plant_hard": "not_proved",
    "fh_plant_no_statement": "no_statement",
}
...
SELFTEST_GOOD = """
FH attempt `fh_plant_easy`: CommRing -> AddCommMagma: PROVED by exact?
FH attempt `fh_plant_hard`: CommRing -> AddCommMagma: not proved by the ladder
FH attempt `fh_plant_no_statement`: CommRing -> AddCommMagma: the rewritten statement is not type-correct after instance re-synthesis — NO STATEMENT
FH attempt `Foo.bar`: Monoid -> MulOneClass: PROVED by simp
FH attempt `Foo.baz`: Monoid -> MulOneClass: not proved by the ladder
FH attempt `Foo.qux`: Monoid -> MulOneClass: 2 source binders match; name a binder index before asking for a verdict — NO STATEMENT
EXIT=0
"""
...
SELFTEST_BAD_PLANT = SELFTEST_GOOD.replace(
    "FH attempt `fh_plant_hard`: CommRing -> AddCommMagma: not proved by the ladder",
    "FH attempt `fh_plant_hard`: CommRing -> AddCommMagma: PROVED by aesop")
...
def selftest() -> int:
    ...
    assert check_plants(v2) == ["fh_plant_hard: proved (must be not_proved)"], \
...
def main() -> int:
    ...
    ap.add_argument("--index", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-attempt-plan.json"))
    ap.add_argument("--log-dir", type=pathlib.Path, default=pathlib.Path("/tmp"),
                    help="where shard logs live, as fh-attempt-<shard name>.log")
    ap.add_argument("--out", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-attempt-scored.json"))
    ...
    for shard in idx["shards"]:
        log = args.log_dir / f"fh-attempt-{shard['name']}.log"
        if not log.exists():
