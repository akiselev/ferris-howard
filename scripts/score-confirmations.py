...
**`#fh_home_refute` forces its class onto every instance binder.** `div_le_one` has three,
...
BLOCK = re.compile(r"^FH home confirm: `(.+?)`$")
LINE = re.compile(r"^\s+\[(.+?)\] -> (.+?): (CONFIRMED|REFUTED|could not rebuild)")


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", type=pathlib.Path, default=pathlib.Path("/tmp/fh-confirm.out"))
    ap.add_argument("--index", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-confirm-index.json"))
    ap.add_argument("--crossing", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-crossing-lattice-shape.json"))
    ap.add_argument("--report", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-precision.json"))
    args = ap.parse_args()
