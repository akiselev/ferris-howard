...
Drives `fh-mcp` over stdio the way a client does — initialize, the `initialized`
...
* `tools/list` advertises the FH-specific tools and *not* the ones delegated to
  `lean-lsp-mcp`, per agent-interface §1's amendment;
* `elaborate` comes back with goals at FH-source spans, which is the thing a generic
...
SERVER = ROOT / "target" / "debug" / "fh-mcp"
...
def main() -> None:
    if not SERVER.exists():
        fail(f"{SERVER} not built — run `cargo build -p fh-atlas --bins`")
    ...
    assert init["serverInfo"]["name"] == "fh-mcp", init
