...
Controls, per §3 and §4: the twelve FH corpus groups are background noise. Group 12's
...
def main() -> int:
    ...
    ap.add_argument("--clusters", type=pathlib.Path, default=pathlib.Path("/tmp/fh-rh.jsonl"))
    ap.add_argument("--controls", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-corpus-nomathlib.jsonl"))
    ap.add_argument("--prepared", type=pathlib.Path, default=pathlib.Path("/tmp/fh-b7.jsonl"))
    ap.add_argument("--out", type=pathlib.Path, default=pathlib.Path("/tmp/fh-b7-report.json"))
    args = ap.parse_args()
