...
The corpus groups are FH's specification: twelve hand-written ports of real mathematics,
...
Usage:
    uv run scripts/corpus-slice.py --full /tmp/fh-mathlib-full.jsonl --out /tmp/fh-merged.jsonl
    uv run scripts/corpus-atlas-experiment.py --slice /tmp/fh-merged.jsonl
"""
...
# ---------------------------------------------------------------------------
# X1 — can the Atlas see FH's output at all?
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# X5 — honesty, over FH's own output
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# X8 — what FH-authored code rests on
# ---------------------------------------------------------------------------

def x8_foundations(c: fa.Corpus, manifest: dict) -> Result:
    r = Result(
        "X8 foundations",
        "FH's expansion is syntax→syntax, so an FH theorem should rest on the same Lean "
        "constants a hand-written one would — a foundation set full of FH constants would "
        "mean the artifact carries an FH dependency, which ADR-006 forbids",
    )
    leaked = {}
    checked = 0
    for g, entry in sorted(manifest.items()):
        for n in entry["authored"]:
            ...
            fh = [x for x in f if x.startswith("FerrisHoward") or x.startswith("FhAtlas")]
            if fh:
                leaked[n] = fh[:5]
    r.check(checked > 0, f"{checked} corpus declarations walked")
    r.check(not leaked,
            f"{len(leaked)} corpus declarations rest on an FH constant "
...
def main() -> int:
    ...
    ap.add_argument("--out", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-corpus-candidates.json"))
    ap.add_argument("--report", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-corpus-report.json"))
