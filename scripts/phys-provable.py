...
def conclusion(enc: bytes) -> tuple[bytes, int]:
    ...
    prefix cannot match one without — `fh_atlas.pyi`'s `Anchor` records the measured cost:
...
class LeanView:
    ...
    def __init__(self, c: fh_atlas.Corpus, unproved: set[str],
...
def census(c: fh_atlas.Corpus, out: dict) -> dict:
...
class Router:
    def __init__(self, v: LeanView, c: fh_atlas.Corpus, wit, unproved: set[str],
                 min_witnesses: int = 1):
        self.v = v
        self.c = c
...
def route_report(r: Router, v: LeanView, name: str, deep: bool, rng,
    ...
    if deep:
        ...
        # `conclusion` is the setting cross-theory analogy needs (`fh_atlas.pyi`, Anchor).
