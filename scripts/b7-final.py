...
def main() -> int:
    ...
    ap.add_argument("--out", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-b7-final.json"))
    args = ap.parse_args()

    prepared = pathlib.Path("/tmp/fh-b7-final.jsonl")
    ...
    dropped = pathlib.Path("/tmp/fh-b7-dropped2.jsonl")
    global DICT_CONFIGS
    ...
    # ---------------- controls --------------------------------------------------------
    t = R.target("CONTROLS", "the twelve FH corpus groups are background noise; "
