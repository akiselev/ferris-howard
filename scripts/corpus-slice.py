...
Usage:
    uv run scripts/corpus-slice.py --slices /tmp/fh-slices --out /tmp/fh-merged.jsonl
"""
...
def main() -> int:
    ...
    for g, rows in sorted(groups.items()):
        ...
        for row in rows:
            ...
            if row.get("stmt"):
                row["stmt"], hits = fh_encoding.rename(row["stmt"], mapping)
                enc_rewrites += hits
            merged.append(row)
        ...
        for row in rows:
            if row.get("stmt"):
                for c in fh_encoding.constants(row["stmt"]):
    ...
    (args.out.parent / "fh-merge-exposure.json").write_text(json.dumps(exposure, indent=2))
