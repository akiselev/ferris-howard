...
def main() -> int:
    ...
    ap.add_argument("--slice", type=pathlib.Path, default=pathlib.Path("/tmp/fh-physlib.jsonl"))
    ap.add_argument("--prepared", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-physlib-theories.jsonl"))
    ap.add_argument("--out", type=pathlib.Path, default=pathlib.Path("/tmp/fh-physlib-report.json"))
    args = ap.parse_args()
    ...
    # `Iff` — a property of the encoded tree. `fh_home.telescope` walks the I3 encoding in
    ...
    for n in names:
        ...
        if d.kind != "theorem" or not d.stmt:
            continue
        try:
            binders, concl = fh_home.telescope(d.stmt)
        except Exception:
            continue
        if concl == "Iff":
            truth_iff.append(n)
