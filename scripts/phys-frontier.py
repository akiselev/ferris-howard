...
## Corpora
    ...
    /tmp/fh-physlib.jsonl              14,563 decls, closure 12.39%   graph-only results
    /tmp/fh-phys-frontier-closed.jsonl 17,067 decls, closure 98.91%   erasure results
    /tmp/fh-phys-frontier-theories.jsonl  the same, modules re-rooted so that a physics
                                          subfield is a theory to `dictionary`/`frontier`

The second is built by `--reduce-from /tmp/fh-physlib-closure.jsonl` (the full
...
`Corpus.closure()` on `/tmp/fh-physlib.jsonl` is **12.39%**: it is `--local`-shaped, so the
...
#
# A scan rather than a parse, for the reason `scripts/fh_encoding.py` gives: names and
...
def scanner_differential(decls, sample=2000, seed=5) -> tuple[int, int]:
    ...
    `scripts/fh_encoding.constants` walks the encoding with different code to a different
    ...
    for x in picked:
        s = scan(decls[x].stmt)
        if s["consts_total"] + s["projs"] != len(fh_encoding.constants(decls[x].stmt)):
            bad += 1
...
def mode_shape(c, decls, path, out):
    ...
    print(f"  scanner differential vs scripts/fh_encoding: {n - bad}/{n} agree"
