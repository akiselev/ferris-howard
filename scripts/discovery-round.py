...
attempt lane's stages out of one round directory:
    ...
    run    fh_batch, N workers      -> per-shard logs, one Mathlib import per worker
...
def main() -> int:
    ...
    ap.add_argument("--workers", type=int, default=3,
                    help="fh_batch processes; each holds Mathlib resident (~9 GB)")
    ap.add_argument("--imports", nargs="+",
                    default=["Mathlib", "FerrisHoward.Atlas.Home"])
    # screen inputs
    ...
    try:
        ...
        if run_stage("run"):
            ...
            sh(manifest, "run", ["lake", "build", "fh_batch",
                                 "FerrisHoward.Atlas.Home"], cwd=LEAN,
            ...
            for w, names in enumerate(interleave(shard_names, args.workers)):
                if not names:
                    continue
                cmd = ["lake", "env", str(LEAN / ".lake" / "build" / "bin" / "fh_batch")]
                for m in args.imports:
                    cmd += ["--import", m]
                cmd += ["--out-dir", str(rd / "logs"), "--log-prefix", "fh-attempt-"]
