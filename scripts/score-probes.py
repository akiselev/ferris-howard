#!/usr/bin/env python3
"""Score `#fh_home_refute` output, keeping the three verdicts apart.

`#fh_home_refute <decl> <class>` forces its class onto **every** instance binder of the
...
HEADER = re.compile(r"FH home confirm: `([^`]+)`")
...
def main() -> int:
    ...
    ap.add_argument("--index", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-gen-index.json"))
