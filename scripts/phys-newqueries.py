...
def telescope_heads(enc: bytes) -> tuple[str | None, list[str]]:
    ...
    `scripts/fh_home.py` establishes the rule this feeds: **a constant whose conclusion is
...
class View:
    ...
    def __init__(self, corpus: fh_atlas.Corpus):
        self.c = corpus
        self.name: list[str] = []
        self.kind: list[str] = []
...
def m1_variants(v: View, c: fh_atlas.Corpus, out: dict, sample: int = 200) -> None:
...
def m3_adjacent(v: View, c: fh_atlas.Corpus, members, out: dict, n_probe: int = 40) -> None:
...
def m4_proof_shape(v: View, c: fh_atlas.Corpus, out: dict) -> None:
...
def m5_hole(v: View, c: fh_atlas.Corpus, out: dict, probes: int = 60) -> None:
...
def main() -> int:
    ...
    t0 = time.time()
    c = fh_atlas.Corpus.load(args.slice)
