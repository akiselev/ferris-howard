...
Nothing here confirms anything. `#fh_home_refute` and the kernel do that, and this script
...
from fh_home import telescope  # noqa: E402
from fh_home_stream import StreamHomeIndex  # noqa: E402
...
`FhAtlas.Home`, not `FerrisHoward.Atlas.Home` — the physics workspace pins v4.32.0 and
cannot depend on the FerrisHoward package (v4.32.2). See the report's engine-change spec:
...
class FastStreamHomeIndex(StreamHomeIndex):
    ...
    **It does not make the sweep faster, and that is the point of keeping it documented.**
    ...
    5.5 s. Essentially all of the 292 s is `fh_home.telescope` walking the encoding in
...
def stage_plan(args) -> int:
    ...
    args.probe_out.write_text(
        HEADER + "\n"
        + "\n".join(f"#fh_home_refute {r['decl']} {r['target']}" for r in chosen) + "\n")
...
def main() -> int:
    ...
    ap.add_argument("--slice", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-physlib-closure.jsonl"))
    ap.add_argument("--unclosed", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-physlib.jsonl"))
    ...
    ap.add_argument("--mathlib-candidates", type=pathlib.Path,
                    default=pathlib.Path("/tmp/fh-gen-closure-candidates.json"))
    ap.add_argument("--mathlib-scored", type=pathlib.Path, nargs="*",
                    default=[pathlib.Path("/tmp/fh-gen-scored-v2.json"),
                             pathlib.Path("/tmp/fh-probeplan-scored.json"),
                             pathlib.Path("/tmp/fh-proberest-scored.json")])
