...
why. The digest is SHA-256 of these bytes, computed Rust-side (`crates/fh-atlas`), because
...
def encodingVersion : String := "fh-stmt-v1"
...
private def theConst (id : Ident) : CommandElabM Name := do
  ...
  match cs with
  | [c] => return c
  | _ => throwErrorAt id "FH: `{id}` does not name exactly one constant"
...
elab "#fh_statement_eq " a:ident b:ident : command => do
  let ea := Atlas.encodeConst (← getEnv) (← theConst a)
  let eb := Atlas.encodeConst (← getEnv) (← theConst b)
  match ea, eb with
  | .ok ea, .ok eb => logInfo (if ea == eb then "same statement" else "different statements")
  | .error e, _ | _, .error e => logError s!"FH: cannot encode: {e}"

/-- Log the canonical statement encoding of a declaration (I3). -/
elab "#fh_statement " id:ident : command => do
  let cs ← liftCoreM <| realizeGlobalConstWithInfos id
  for c in cs do
    match Atlas.encodeConst (← getEnv) c with
    | .ok s => logInfo s
    | .error e => logError s!"FH: cannot encode `{c}`: {e}"
