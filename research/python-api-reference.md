# fh-atlas Python API Reference (Static Surface), v0.1-draft
...
```python
fh_atlas.require(spec: str) -> None            # version gate: fa.require(">=0.3,<0.5"); raises VersionConflict
fh_atlas.config(**kw) -> Config                # process-wide: cache dirs, lean toolchain pin, worker counts
fh_atlas.campaign(name: str, *, resume: bool = True) -> Campaign   # context manager; §8
fh_atlas.replay(journal: Path | Journal, *, strict: bool = True) -> ReplayReport
fh_atlas.__version__: str
fh_atlas.__corpus_abi__: int                   # bumped when index formats change; pinned into snapshots
```
