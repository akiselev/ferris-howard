...
`#fh_home_refute`/`#fh_home_attempt` can only reach physics declarations from here. The
old location, `lean/FerrisHoward/Atlas/Home.lean`, is a one-line re-export shim so every
existing import site and generated probe file keeps working. The Lean *namespace* stays
`FerrisHoward.Atlas` while the module path is `FhAtlas.Home`, per the package's naming
...
`#fh_home_confirm` and `#fh_home_refute` ask whether the declaration's **own proof term**
...
`#fh_home_attempt` closes that. It builds the weakened statement and hands it to a **tactic
...
private def tryTactic (ty : Expr) (levelParams : List Name)
  ...
  try
    ...
    -- vanished — caught by the per-shard `fh_plant_hard` control on the first census
    ...
    tryCatchRuntimeEx (do
      ...
      return ((← getEnv).addDeclCore 0 decl none).toOption.isSome)
      fun _ => return false
...
/-- `#fh_home_attempt <decl> <source> => <target> by tac, tac, …` — state the exact
...
elab "#fh_home_attempt " n:ident source:ident " => " c:ident " by " tacs:tactic,+ : command => do
  let name ← liftCoreM <| realizeGlobalConstNoOverload n
  ...
  let ty0 ←
    ...
    | #[ty] => pure ty
    | #[] =>
      logInfo s!"FH attempt `{name}`: {source} -> {cls}: source binder not found or the target cannot be applied to its arguments — NO STATEMENT"
      return
    | _ =>
      logInfo s!"FH attempt `{name}`: {source} -> {cls}: {built.size} source binders match; name a binder index before asking for a verdict — NO STATEMENT"
      return
  ...
  let some ty := ty?
    | logInfo s!"FH attempt `{name}`: {source} -> {cls}: the rewritten statement could not re-synthesise its instance arguments — NO STATEMENT"
      return
  unless ← liftTermElabM (tryCatchRuntimeEx (isTypeCorrect ty) fun _ => pure false) do
    logInfo s!"FH attempt `{name}`: {source} -> {cls}: the rewritten statement is not type-correct after instance re-synthesis — NO STATEMENT"
    return
  ...
  match won with
  | some tac => logInfo s!"FH attempt `{name}`: {source} -> {cls}: PROVED by {tac}"
  | none => logInfo s!"FH attempt `{name}`: {source} -> {cls}: not proved by the ladder"
